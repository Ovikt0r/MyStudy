...
public class Ex13_GreenhouseLinkedList {
    public static void main(String[] args) {
        GreenhouseControls ghc = new GreenhouseControls();
        ghc.addEvent(ghc.new Bell(900));
        Event[] eventList = {
                ghc.new ThermostatDay(0),
                ghc.new LightOn(200),
                ghc.new LightOff(400),
                ghc.new WaterOff(600),
                ghc.new WaterOn(800),
                ghc.new ThermostatDay(1400),
                ghc.new ThermostatNight(1600)
        };
        ghc.addEvent(ghc.new Restart(2000, eventList));
        if(args.length == 1)
            ghc.addEvent(new GreenhouseControls.Terminate(parseInt(args[0])));
        ghc.run();
    }
}